...
class Launcher {
    ...
    runSpecs () {
        ...
        console.log('after run spec', this.getNumberOfRunningInstances(), this.getNumberOfSpecsLeft());
    ...
    startInstance (specs, caps, cid, server) {
        ...
        const childProcessSent = childProcess.send({
            cid,
            command: 'run',
        ...
        if(!childProcessSent){
            console.log(cid, 'launcher failed to send run');
        }
