...
class Runner {
    ...
    async run (m) {
         ...
         */
        const runnerStart = process.send({
            event: 'runner:start',
            cid: m.cid,
            specs: m.specs,
        ...
        if(!runnerStart){
            console.log(m.cid, 'failed to send runner:start');
        }
    ...
    async end (failures = 0, inWatchMode = this.inWatchMode, sendProcessEvent = true) {
        ...
        const runnerEndSent = process.send({
            event: 'runner:end',
            failures: failures,
            cid: this.cid,
            specs: this.specs
